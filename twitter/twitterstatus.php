	...
	public function Render() {
		...
		if ($cacheage < 0 || $cacheage > $this->CacheFor) {
			...
			if ($json) {
				...
				if(strpos($status,'tinyurl.com')){
					$status = str_replace('<',' <',$status);
					$status = str_replace('>','> ',$status);
					$statusParts = explode(' ',$status);
					for($i=0;$i<count($statusParts);$i++){
						if(strstr($statusParts[$i],'tinyurl.com') && strpos($statusParts[$i],'http') !== 0){
							$statusParts[$i] = '<a href="'.$statusParts[$i].'" target="_blank">'.$statusParts[$i].'</a>';
						}
					}
					$status = implode(' ',$statusParts);
				}
				$status = str_replace('tinyurl.com','http://tinyurl.com',$status);
				// parse Twitter links
